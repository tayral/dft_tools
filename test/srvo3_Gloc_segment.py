...
from pytriqs.applications.dft.solver_multiband_segment import *
block_names = [i for (i,j) in SK.gf_struct_solver[0]]
S=SolverMultiBandSegment(beta=Beta,n_orb=Norb,block_names=block_names,map=SK.map[0], use_matrix=useMatrix, U_interact=U, J_hund=J)

SK.put_Sigma([S.Sigma_iw])
Gloc=SK.extract_G_loc()

ar = HDFArchive('srvo3_Gloc_segment.output.h5','w')
ar['Gloc'] = Gloc[0]
